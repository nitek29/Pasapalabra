        ...
        return Observer { listOfWorkInfo ->
            ...
            for (workInfo in listOfWorkInfo.indices) {
                if (listOfWorkInfo[workInfo].state.isFinished) {
                    ...
                    when (tag) {
                        TAG_TRANSLATOR -> {
                            ...
                            if (!outputImageUri.isNullOrEmpty()) {
                                toolchain.get(workInfo+1).output = outputImageUri
                                Log.d("Activity", "TAG : $tag\t workinfo :$workInfo+1 \toutput : ${outputImageUri} \t lang ${toolchain.get(workInfo+1).title}\t output : ${toolchain.get(workInfo+1).output} ")
                            }
                        }
                        TAG_STT -> {
                            outputImageUri = listOfWorkInfo[workInfo].outputData.getString(KEY_STT)
                        ...
                        TAG_OUTPUT -> {
                            ...
                            if (!outputImageUri.isNullOrEmpty()) {
                                ...
                                Log.d("Activity", "TAG : $tag\t workinfo : ${toolchain.size-1}\t output : ${outputImageUri} \t lang ${toolchain.get(workInfo).title}\t output : ${toolchain.get(toolchain.size-1).output} ")
                            }
                        }
                    }
                    ...
                    //showWorkInProgress()
                    //Log.d("Activity", "worker work not finisehd")
                }
            }
        }
    }
