    ...
    private fun workInfosObserver2(toolchain: ToolChain, pos: Int): Observer<List<WorkInfo>> {
        return Observer { listOfWorkInfo ->
            ...
            if (workInfo.state.isFinished) {
                ...
                // Normally this processing, which is not directly related to drawing views on
                // screen would be in the ViewModel. For simplicity we are keeping it here.
                val outputText = workInfo.outputData.getString(KEY_STT)

                // If there is an output file show "See File" button
                if (!outputText.isNullOrEmpty()) {
                    toolchain.get(pos + 1).output = outputText
                    Log.d("Activity", "TAG : $pos\t workinfo :${pos + 1} \toutput : ${outputText} \t lang ${toolchain.get(pos + 1).title}\t output : ${toolchain.get(pos + 1).output} ")
                }
                tool_chain_list.adapter = this.tool_chain_list_adapter
            }
        }
